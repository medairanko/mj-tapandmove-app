...
export function findBestEntityMatch(spokenRemainder, entities) {
  ...
  if (exactMatches.length > 0) {
    ...
    const topLength = normalize(exactMatches[0].friendly_name).length;
    const tiedAtTop = exactMatches.filter((entity) => normalize(entity.friendly_name).length === topLength);
